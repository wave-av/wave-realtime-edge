...
| `POST` | `/rtk/join` | Gateway (`x-wave-internal`) | **Live** | RealtimeKit meeting create + participant token mint |
| `POST` | `/rtk/turn` | Gateway (`x-wave-internal`) | **Live** | TURN/ICE credential mint (NAT traversal) |
| `POST` | `/rtk/recording-webhook` | `rtk-signature` (RSA-SHA256) | **Live** | Recording status update (managed PULL puller) |
...
never reads or validates it. For gateway-sealed routes such as `/rtk/join` and
`/rtk/turn`, the gateway also stamps `x-wave-internal` (and `x-wave-org`) when
`WAVE_INTERNAL_SECRET` is configured; direct callers must not forge this header.
Local and test environments without that secret do not require the seal.

`POST /rtk/recording-webhook` is the exception: it is called directly by
RealtimeKit and authenticates the raw request body with an `rtk-signature` RSA-SHA256
signature. It does not use `Authorization: Bearer`.
