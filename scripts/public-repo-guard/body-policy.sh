...
check() {
  ...
  # Same fail-closed contract as the primary scan: rg exit 0/1 is match/no-match,
  # >=2 is a real error. A filter that errors out must not empty `matches` and
  # turn a hit into a silent pass.
  local matches frc
