...
import { handleEgressArmRoute, maybeHandleEgressArmRoute, type EgressArmRouteEnv } from "../src/egress-arm-route.js";
...
describe("POST /v1/egress/teardown", () => {
  it("calls deleteOutput and emits the rtmp-out meter", async () => {
    ...
    const res = await handleEgressArmRoute(
      ...
      baseEnv({ GATEWAY_BASE_URL: "https://api.wave.online", WAVE_SERVICE_TOKEN: "svc-tok" }),
      "acme",
      { fetchFn },
    );
    ...
    expect(deleteCall?.url).toContain(`/live_inputs/${SOURCE_UID}/outputs/lo-42`);
  ...
  it("is idempotent — a CF 404 on delete is still ok:true (already gone)", async () => {
    ...
    const res = await handleEgressArmRoute(
      new Request("https://edge/v1/egress/teardown", {
        method: "POST",
        body: JSON.stringify({ inputId: SOURCE_UID, outputId: "lo-gone", org: "acme", meetingUuid: "mtg-1" }),
      }),
      baseEnv(),
      "acme",
      { fetchFn },
  ...
  it("stays INERT for metering (no /v1/internal/usage call) when GATEWAY_BASE_URL/WAVE_SERVICE_TOKEN are unprovisioned", async () => {
    ...
    const res = await handleEgressArmRoute(
      new Request("https://edge/v1/egress/teardown", {
        method: "POST",
        body: JSON.stringify({ inputId: SOURCE_UID, outputId: "lo-42", org: "acme", meetingUuid: "mtg-1", durationMs: 30_000 }),
      }),
      baseEnv(), // no GATEWAY_BASE_URL / WAVE_SERVICE_TOKEN
      "acme",
      { fetchFn },
    );
    expect(res?.status).toBe(200);
