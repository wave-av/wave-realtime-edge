...
describe("RoomDO.fetch — leave emits metering when provisioned", () => {
	it("flushes participant usage to the gateway /v1/internal/usage on leave", async () => {
		...
		// Metered time is `leftAt - joinedAt` (metering.ts connectedMinutes), and a usage row is emitted
		// only when that difference is STRICTLY > 0. join/publish/leave below are pure in-memory calls that
		// routinely complete inside a single millisecond, which made this assertion intermittently fail
		// on slower runs. Pin the clock instead of racing it: only Date is faked, so setTimeout and
		// queueMicrotask stay real and nothing that awaits a real timer can hang.
		...
		try {
			...
			// 5 minutes of session before the leave: a real metered window, not a sub-ms accident.
