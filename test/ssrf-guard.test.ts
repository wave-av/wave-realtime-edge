...
describe("validateDestinationUrl — deny matrix", () => {
  const denyCases: Array<[string, string, string]> = [
    ...
    ["link-local / metadata IP", "rtmp://169.254.169.254:1935/live", "link-local"],
  ...
  for (const [name, url, expectedFrag] of denyCases) {
    it(`denies ${name} (${url})`, async () => {
      const res = await validateDestinationUrl("rtmp", url);
