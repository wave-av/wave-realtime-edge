...
describe("ContainerBridgeIngestBackend — refuses a source that can't be admitted", () => {
  ...
  it("SSRF: is `unroutable` for an inbound host on the WAVE tailnet (100.64/10 CGNAT)", async () => {
    const client = fakeClient();
    const be = new ContainerBridgeIngestBackend(client);
    const outcome = await be.admit(job("moqPush"), { ...CTX, inbound: { host: "100.79.59.52" } });
    expect(outcome.status).toBe("unroutable");
    expect(client.calls).toHaveLength(0);
  });
...
describe("guardIngestHost / isSafeRemoteHost — the SSRF host guard", () => {
  it("blocks IPv4 private / loopback / link-local / CGNAT / reserved ranges", () => {
    for (const bad of ["10.0.0.1", "172.16.5.4", "192.168.1.1", "127.0.0.1", "169.254.169.254", "100.100.0.1", "0.0.0.0", "224.0.0.1", "255.255.255.255", "198.18.0.1", "192.0.2.5", "203.0.113.7"])
