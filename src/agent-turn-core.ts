 ...
 */
import { AgentSessionError, AgentMediaDeps } from "./agent-session.js";
import { decodePacket } from "./encoders/container-adapter.js";
import { type IngestFraming } from "./agent-ingest-adapter.js";
import { SpeechSession } from "./agent-turn-speech.js";
import { LatencyCollector, type TurnHopMarks } from "./turn-latency.js";
import { meterFinishedTurn, meterSessionClose } from "./agent-turn-meter.js";
import { TurnCogsLedger } from "./voice-cogs-ledger.js";
import type { VoiceCogsRates } from "./voice-cogs.js";
import { Vad, VadConfig } from "./agent-vad.js";
import { ToolAllowlist, ToolUse, ToolResult } from "./agent-tools.js";
import { buildTurnSystemPrompt } from "./agent-turn-providers.js";

// ── Public contracts (the injectable-deps seam) ──────────────────────────────────────────────────────────────

import { runAgentTurn } from "./agent-turn-run.js";
import { executeTurnTools } from "./agent-turn-tools-exec.js";
import { UtteranceBuffer } from "./agent-turn-utterance.js";
import { DEFAULT_MAX_TOOL_ITERATIONS, DEFAULT_TTS_LEAD_MS } from "./turn-config.js";
...
export class TurnTakingCore {
  ...
  /** The bounded utterance buffer (agent-turn-utterance.ts owns the drop-oldest eviction law). */
  private readonly utteranceBuf = new UtteranceBuffer();
  private outSeq = 0;
  ...
  async onFrame(frame: Uint8Array): Promise<void> {
    ...
    try {
      ...
      stage = "stt";
      const pcm = this.utteranceBuf.drain();
  ...
  /**
   * Run ONE turn for a final user transcript. The BOUNDED AGENTIC TOOL LOOP (step 5) and its
   * history-atomicity law live in agent-turn-run.ts, documented at the loop itself (moved with
   * the code, 2026-08-30 decompose).
   */
  ...
  private async executeTools(toolUses: ToolUse[]): Promise<ToolResult[]> {
    // EXECUTION policy lives in agent-turn-tools-exec.ts (token-budget decompose, 2026-08-30):
    // the allowlist refusal, the fail-safe error shape, the redacted audit. Delegation keeps the
    // class's private surface; the loop reaches this through TurnRunContext.executeTools.
    return executeTurnTools(
      { deps: this.deps, tools: this.tools, aborted: () => this.aborted, idFields: () => this.idFields() },
      toolUses,
    );
  }
  ...
  private pushUtterance(payload: Uint8Array): void {
    this.utteranceBuf.push(payload);
  }

  private resetUtterance(): void {
    this.utteranceBuf.reset();
  }
