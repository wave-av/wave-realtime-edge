...
export async function createPublisher({ sfuBase, appId, appSecret, wavPath, trackName = "harness-mic", udpPort = 5004, loops = -1, log = () => {} }) {
  ...
  // One ffmpeg → Opus/RTP/UDP. Factored so `bargeSwap` (Leg 4) can RESTART the source on the SAME track (same
  // ssrc → the SFU/agent see a continuing talkspurt) to inject a barge-in utterance over the speaking agent.
  const spawnFf = (path, lp) => {
    const p = spawn("ffmpeg", [
      "-hide_banner", "-loglevel", "error", "-re", "-stream_loop", String(lp), "-i", path,
      "-c:a", "libopus", "-ar", "48000", "-ac", "2", "-application", "voip",
      "-payload_type", String(OPUS_PT), "-ssrc", "11111111",
      "-f", "rtp", `rtp://127.0.0.1:${udpPort}`,
    ]);
    p.stderr.on("data", (d) => log("ffmpeg", { err: String(d).slice(0, 200) }));
    return p;
  };
  let ff = spawnFf(wavPath, loops);
  ...
  // Leg 4 barge-in: kill the current source and restart ffmpeg with a fresh loud utterance on the SAME track.
  // After utterance #1's noise tail fired the agent VAD's speech-end (turn ran), the source goes silent (ffmpeg
  // exits with loops=0). This RE-arms loud audio → a fresh VAD onset → speech-start → bargeIn() aborts the turn.
  const bargeSwap = (path, lp = 0) => {
  ...
  return { sessionId, trackName, pc, connected, stop, bargeSwap, rtpSent: () => rtpCount };
}
